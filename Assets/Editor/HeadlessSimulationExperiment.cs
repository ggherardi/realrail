...
namespace RealRail.Editor
{
    ...
    public sealed class HeadlessSimulationExperiment
    {
        ...
        public int TotalJobs => _coordinator.Executions.Count;
        public int TerminalJobs { get { var count = 0; foreach (var execution in _coordinator.Executions) if (execution.State != SimulationBatchProcessState.Queued && execution.State != SimulationBatchProcessState.Running) count++; return count; } }
        ...
        public int ActiveWorkers => _coordinator.ActiveCount;
        public int FailureCount { get { var count = 0; foreach (var execution in _coordinator.Executions) if (execution.State == SimulationBatchProcessState.Failed || execution.State == SimulationBatchProcessState.TimedOut || execution.State == SimulationBatchProcessState.Cancelled) count++; return count; } }
        ...
        public HeadlessSimulationExperiment(BalanceExperimentDefinition definition, float speed, int workers)
        {
            ...
            _experimentId = Guid.NewGuid().ToString("N");
            ...
            _coordinator = new SimulationBatchProcessCoordinator(projects.Length, Path.Combine(_root, "requests"), i => projects[i], execution => new UnityBatchWorkerCommand { FileName = UnityEditorExecutablePath(), ExecuteMethod = "RealRail.Editor.SimulationBatchWorker.Execute", LogPath = Path.Combine(_root, "logs", execution.Request.jobId + ".log") });
            BuildAndEnqueue();
        ...
        void BuildAndEnqueue()
        {
            ...
            { var id = Guid.NewGuid().ToString("N"); _coordinator.Enqueue(new SimulationBatchRequest(id, seed, p.Profile, _speed, 300f, c.Configuration, _experimentId, c.Id), Path.Combine(_root, "results", id + ".json")); }
        }
        void FinishSuccess()
        {
        ...
        static string UnityEditorExecutablePath()
        {
            var applicationPath = EditorApplication.applicationPath;
#if UNITY_EDITOR_OSX
            if (Directory.Exists(applicationPath)) return Path.Combine(applicationPath, "Contents", "MacOS", "Unity");
#endif
