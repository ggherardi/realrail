...
namespace RealRail.Tests
{
    public sealed class CameraCompositionTests
    {
        ...
        public void AuthoredCamera_FocusesTheArenaWhileKeepingThePlayerAndSpawnEdgeVisible()
        {
            ...
            Assert.AreEqual(new Vector3(0f, 13f, -11f), camera.transform.position);
            Assert.That(Mathf.DeltaAngle(34f, camera.transform.eulerAngles.x), Is.EqualTo(0f).Within(0.01f));
            Assert.AreEqual(42f, camera.fieldOfView);
            ...
            Assert.That(playerViewport.y, Is.InRange(0.05f, 0.25f));
            ...
            for (var lane = 0; lane < lanes.LaneCount; lane++)
            {
                ...
                Assert.That(spawnViewport.y, Is.InRange(0.8f, 1f));
